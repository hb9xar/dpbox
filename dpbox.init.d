...
case "$1" in
	...
	startproc -q /usr/local/sbin/dpbox -i /etc/dpbox/dpbox.ini || return=$rc_failed

	echo -e "$return"
	;;
    stop)
	echo -n "Shutting down service dpbox"
      	killproc -TERM /usr/local/sbin/dpbox || return=$rc_failed
        echo -e "$return"
	;;
    restart)
	...
	killproc -q -HUP /usr/local/sbin/dpbox || return=$rc_failed
        echo -e "$return"
	;;
    status)
	...
	checkproc /usr/local/sbin/dpbox && echo OK || echo No process
	;;
    *)
