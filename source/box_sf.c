 ...
 * Date		Who	Comment
 *----------------------------------------------------------------
 * 28.10.02	HB9XAR	if a 'MYBBS NO' statement is present in .SF file
 *			don't exchange any mybbs info (no WP, no EM, nada)
 *
 *
 */
  ...
  while (result == 0) {
    ...
    *sfps     	      	= '\0';
	...
	while (file2str(k, hs)) {
	  ...
	  if (!strcmp(w, "MYBBS")) {
/* this is a bloody hack for testing something...  HB9XAR*/
	    hp->send_em = true;
      	    if (hp->em == EM_WPROT) {
	      get_word(hs, w);
	      if (!*w) continue;
	      if (!strcmp(w, "WP")) hp->em = EM_WP;
	      if (!strcmp(w, "EM")) hp->em = EM_EM;
	      if (!strcmp(w, "NO")) { /*&&&&start*/
	        hp->send_em	= false;
	        hp->em		= EM_UNKNOWN;
	      } /*&&&& end*/
	      continue;
  ...
  char *bid, boolean *rubrik_transfer, char *lastvias)
{
  ...
  *rubrik_transfer = false;
  ...
  /* if this is a WP and we have sfp->send_em == FALSE, refuse it */
  /* In the .SF files, a MYBBS NO will switch off sendding of MYBBS info */
  if (is_wp && !sfp->send_em) ok = false;
