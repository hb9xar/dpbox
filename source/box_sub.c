...
//&&&&hb9xar use snprintf (don't overwrite stack in case of fup)
//    sprintf(STR1, "%s : [hc]invalid header checksum", h.dest);
    snprintf(STR1, sizeof(STR1)-1, "%20.20s : [hc]invalid header checksum", h.dest);
    debug(0, -1, 137, STR1);
    return false;
  }
  ...
  sprintf(STR1, "%s : [hv]invalid header version", h.dest);
  debug(0, -1, 137, STR1);
  return false;
}
